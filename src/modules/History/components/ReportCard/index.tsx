...
export default function ReportCard() {
    ...
    const [search, setSearch] = useState("") // 🔥 Search state ishlaydi
    const [from, setFrom] = useState<string | null>(defaultFrom)
    const [until, setUntil] = useState<string | null>(defaultUntil)

    const { data } = useGetHistoryListQuery({ page, from: from || "", until: until || "", search })
                    ...
                    {data?.data?.map((item) => (
                        ...
                        >
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-[#5B9BEC] text-[14px] font-bold">{formatDate(item.created_at)}</span>
                            </div>
